...
for file in jsx_files:
    ...
    def replacer(match):
        ...
        brand = match.group(7).strip().replace('"', '&quot;')
        title = match.group(9).strip().replace('"', '&quot;')
        price_html = match.group(11).strip()
        ...
        # return the same matched group block but with the heart tag replaced
        # re.sub replaces the entire matched string (match.group(0)) with the return value
